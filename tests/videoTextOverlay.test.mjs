...
assert.equal(validateOverlayVideoFile({ type: 'image/gif', size: 1024 }), '')
assert.equal(validateOverlayVideoFile({ type: '', name: 'downloaded.MOV', size: 1024 }), '')
assert.match(validateOverlayVideoFile({ type: 'image/png', size: 1024 }), /MP4、MOV、WebM 或 GIF/)
assert.match(validateOverlayVideoFile({ type: 'video/mp4', size: 91 * 1024 * 1024 }), /90MB/)
