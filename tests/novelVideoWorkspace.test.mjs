...
assert.match(workspace, /新建小说任务/)
assert.match(workspace, /startNewJob/)
assert.match(workspace, /removeItem\(RECENT_JOB_KEY\)/)
assert.match(workspace, /restoreRememberedJob/)
assert.match(workspace, /getNovelVideoJob\(remembered, \{ signal: request\.signal \}\)/)
assert.match(workspace, /response\?\.status === 404/)
assert.match(workspace, /finally \{ if \(request\.generation === restoreGeneration\) recentLoading\.value = false \}[\s\S]*if \(restoreRemembered && request\.generation === restoreGeneration\) await restoreRememberedJob\(request\)/)
assert.match(workspace, /editableShots\.value = \[\][\s\S]*emit\('new-job'\)/)
assert.match(workspace, /restoreGeneration/)
assert.match(workspace, /AbortController/)
assert.match(workspace, /request\.generation === restoreGeneration/)
assert.match(workspace, /cancelRestoreRequests/)
...
assert.match(workspace, /listNovelVideoJobs/)
assert.match(workspace, /最近小说任务/)
assert.match(workspace, /恢复任务/)
assert.match(workspace, /refreshRecentJobs/)
