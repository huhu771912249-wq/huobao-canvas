...
assert.match(workspace, /getNovelVideoJob\(remembered\)/)
assert.match(workspace, /response\?\.status === 404/)
assert.match(workspace, /finally \{ recentLoading\.value = false \}[\s\S]*if \(restoreRemembered\) await restoreRememberedJob\(\)/)
