  ...
  <main class="gif-editor-page">
    <header class="editor-header">
      <div class="header-left">
        <button class="icon-button" type="button" title="返回首页" @click="router.push('/')">←</button>
        <div class="brand-mark">GIF</div>
        <div>
          <div class="eyebrow">冠希 CREATIVE CUT</div>
          <div class="project-line"><input v-model="projectName" aria-label="项目名称"><span>已自动保存</span></div>
        </div>
      </div>
...
const projectName = ref('新品投放 GIF 方案')
...
const imageTracks = ref([
  { id: 'image-1', name: '品牌 Logo.png', start: 0, end: 11, x: 82, y: 10, size: 22, url: '' }
])
...
const importImage = event => {
  ...
  const url = URL.createObjectURL(file); objectUrls.push(url)
  const item = { id: `image-${++sequence}`, name: file.name, start: playhead.value, end: Math.min(totalDuration.value, playhead.value + 4), x: 75, y: 18, size: 24, url }
...
const imageOverlayStyle = item => ({ left: `${item.x}%`, top: `${item.y}%`, width: `${item.size}%` })
