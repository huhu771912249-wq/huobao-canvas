...
const importMedia = async event => {
  ...
  try {
    for (const file of files) {
      ...
      const clip = { id: `clip-${++sequence}`, name: asset.name || file.name, kind, duration, color: 'linear-gradient(135deg,#172554,#2563eb,#67e8f9)', transition: '叠化', url: asset.url, mime: asset.mime, width: asset.width, height: asset.height }
