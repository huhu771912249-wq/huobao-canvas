    ...
    <template v-else>
      <div class="grid gap-4 xl:grid-cols-[1fr_320px]">
        <div class="rounded-2xl border border-slate-700 bg-slate-900/60 p-5">
          ...
          <div class="mt-4 grid gap-3 text-xs sm:grid-cols-3"><div class="metric"><span>原生分辨率</span><b>{{ nativeResolution }}</b></div><div class="metric"><span>AI 超分</span><b>{{ qualityMode === 'quality' ? 'SeedVR2 3B FP16' : '不启用' }}</b></div><div class="metric"><span>最终输出</span><b>{{ finalResolution }}</b></div></div>
...
const stopPolling = () => { pollGeneration += 1; polling.value = false; if (pollTimer) clearTimeout(pollTimer); pollTimer = null }
