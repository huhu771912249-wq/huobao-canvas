...
const startNewJob = () => { stopPolling(); job.value = null; subtitles.value = []; subtitleDirty.value = false; subtitleError.value = ''; retryingShotIds.value = new Set(); restoringJobId.value = ''; error.value = ''; qualityMode.value = 'quality'; clearRememberedJob(); editableShots.value = cloneShots(props.storyboard?.shots); dirty.value = Boolean(editableShots.value.length); emit('new-job') }
