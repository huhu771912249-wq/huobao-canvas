...
const restoreRememberedJob = async () => { let remembered = ''; try { remembered = window.localStorage?.getItem(RECENT_JOB_KEY) || '' } catch { remembered = '' } if (!remembered || job.value || props.storyboard) return; try { applyRestoredJob(await getNovelVideoJob(remembered)) } catch (cause) { if (cause?.response?.status === 404) clearRememberedJob(); else recentError.value = errorMessage(cause) } }
const refreshRecentJobs = async ({ restoreRemembered = false } = {}) => { recentLoading.value = true; recentError.value = ''; try { const result = await listNovelVideoJobs({ limit: 50, cursor: 0 }); recentJobs.value = Array.isArray(result?.jobs) ? result.jobs : [] } catch (cause) { recentError.value = errorMessage(cause) } finally { recentLoading.value = false } if (restoreRemembered) await restoreRememberedJob() }
const restoreJob = async jobId => { stopPolling(); restoringJobId.value = jobId; error.value = ''; try { applyRestoredJob(await getNovelVideoJob(jobId)) } catch (cause) { error.value = errorMessage(cause) } finally { restoringJobId.value = '' } }
const startNewJob = () => { stopPolling(); job.value = null; subtitles.value = []; subtitleDirty.value = false; subtitleError.value = ''; retryingShotIds.value = new Set(); restoringJobId.value = ''; error.value = ''; qualityMode.value = 'quality'; clearRememberedJob(); editableShots.value = []; dirty.value = false; emit('new-job') }
