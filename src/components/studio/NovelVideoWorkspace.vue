...
let restoreGeneration = 0; let restoreController = null
...
const clearRememberedJob = () => { try { window.localStorage?.removeItem(RECENT_JOB_KEY) } catch { /* browser privacy mode */ } }
const beginRestoreRequest = () => { restoreGeneration += 1; restoreController?.abort(); restoreController = new AbortController(); return { generation: restoreGeneration, signal: restoreController.signal } }
const cancelRestoreRequests = () => { restoreGeneration += 1; restoreController?.abort(); restoreController = null; restoringJobId.value = '' }
const applyRestoredJob = restored => { syncJob(restored); editableShots.value = cloneShots(restored.shots); dirty.value = false; qualityMode.value = restored?.quality_profile?.mode === 'fast' ? 'fast' : 'quality'; rememberJob(restored.job_id); if (shouldPollNovelJob(restored)) resumePolling() }
const restoreRememberedJob = async context => { let remembered = ''; try { remembered = window.localStorage?.getItem(RECENT_JOB_KEY) || '' } catch { remembered = '' } if (!remembered || job.value || props.storyboard) return; const request = context || beginRestoreRequest(); try { const restored = await getNovelVideoJob(remembered, { signal: request.signal }); if (request.generation === restoreGeneration) applyRestoredJob(restored) } catch (cause) { if (request.generation !== restoreGeneration) return; if (cause?.response?.status === 404) clearRememberedJob(); else if (cause?.name !== 'CanceledError' && cause?.name !== 'AbortError') recentError.value = errorMessage(cause) } }
const refreshRecentJobs = async ({ restoreRemembered = false } = {}) => { const request = beginRestoreRequest(); recentLoading.value = true; recentError.value = ''; try { const result = await listNovelVideoJobs({ limit: 50, signal: request.signal }); if (request.generation === restoreGeneration) recentJobs.value = Array.isArray(result?.jobs) ? result.jobs : [] } catch (cause) { if (request.generation === restoreGeneration && cause?.name !== 'CanceledError' && cause?.name !== 'AbortError') recentError.value = errorMessage(cause) } finally { if (request.generation === restoreGeneration) recentLoading.value = false } if (restoreRemembered && request.generation === restoreGeneration) await restoreRememberedJob(request) }
const restoreJob = async jobId => { stopPolling(); const request = beginRestoreRequest(); restoringJobId.value = jobId; error.value = ''; try { const restored = await getNovelVideoJob(jobId, { signal: request.signal }); if (request.generation === restoreGeneration) applyRestoredJob(restored) } catch (cause) { if (request.generation === restoreGeneration && cause?.name !== 'CanceledError' && cause?.name !== 'AbortError') error.value = errorMessage(cause) } finally { if (request.generation === restoreGeneration) restoringJobId.value = '' } }
const startNewJob = () => { stopPolling(); cancelRestoreRequests(); job.value = null; subtitles.value = []; subtitleDirty.value = false; subtitleError.value = ''; retryingShotIds.value = new Set(); error.value = ''; qualityMode.value = 'quality'; clearRememberedJob(); editableShots.value = []; dirty.value = false; emit('new-job') }
