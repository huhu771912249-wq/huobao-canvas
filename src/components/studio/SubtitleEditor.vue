  ...
  <section class="rounded-2xl border border-slate-700 bg-slate-950/50 p-4">
    ...
    <div class="max-h-80 space-y-2 overflow-auto">
      <div v-for="(segment, index) in segments" :key="segment.id || index" class="grid gap-2 rounded-xl border border-slate-800 p-3 md:grid-cols-[90px_90px_120px_1fr]">
        <label class="text-xs text-slate-400">开始<input v-model.number="segment.start" :disabled="saving" type="number" min="0" step="0.1" class="subtitle-input" @input="$emit('change')" /></label>
        <label class="text-xs text-slate-400">结束<input v-model.number="segment.end" :disabled="saving" type="number" min="0" step="0.1" class="subtitle-input" @input="$emit('change')" /></label>
        <label class="text-xs text-slate-400">说话人<input v-model="segment.speaker" :disabled="saving" maxlength="200" class="subtitle-input" @input="$emit('change')" /></label>
        <label class="text-xs text-slate-400">字幕<input v-model="segment.text" :disabled="saving" class="subtitle-input" @input="$emit('change')" /></label>
      </div>
    </div>
